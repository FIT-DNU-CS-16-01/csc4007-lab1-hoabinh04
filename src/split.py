...
def make_splits(df: pd.DataFrame, seed: int, train_ratio: float=0.5, val_ratio: float=0.0, test_ratio: float=0.5):
    ...
    # Handle edge case: no validation set
    if val_ratio == 0:
        strat_temp = df_temp["label"] if df_temp["label"].nunique() > 1 else None
        df_val, df_test = pd.DataFrame(columns=df.columns), df_temp
    else:
